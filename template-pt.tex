...
\clearpage
\section{Employment Document}
\instr{
  \begin{itemize}
    \item Include here a copy of the candidate’s most recent, signed Faculty
      Contract, initial offer letter, or promotion letter as appropriate.
    \item Add any addendums to the faculty members employment terms that
      occurred after the date of the document in 1.
  \end{itemize}

  If the candidate or department does not have a copy of the Faculty Contract,
...
\newsavebox\precite
\savebox\precite{\parbox{\textwidth}{\input{template-pt-pubinc}}}
...
\instr{
Data required for the expanded curriculum vitae should largely be available in
previous Faculty Data Reports.
}
...
\instr{
  Table need only be completed for promotion dossiers of Research and Library faculty if it is relevant to the candidate’s application package.
}
...
\instr{
  ...
  As directed by the Faculty Handbook, the Departmental
...
\instr{
  ...
  Include here a brief overview of the process used to solicit external
  references, define how outside referees were identified, and should include a
  copy of the generic letter used for the solicitation.

  If dossier is for promotion consideration of teaching faculty, include the
  following sentence here, ``Dossier for promotion consideration of a teaching
  faculty member, external letters are not required.'' Leave table below blank,
  and remove the page 11.
}

\instr{
  Section is not required for Teaching Faculty promotion dossiers. If a Teaching
  Faculty member wants to include external letters in his/her dossier, these
  should be included in Section 9, ``Other Information.''
}
...
\instr{
  ...
  \item include metrics such as number of citations, h-index over entire career,
    and h-index while at CSM. H-index measurements cited should include both
    those derived from ``Web of Science'' and those derived from ``Google Scholar.''
...
Provide information on the competitiveness of the funding sources.
...
\instr{
  For Teaching and Library faculty for whom the section is not relevant, include
  on this page the following sentence; ``Not relevant for promotion consideration
  of faculty member.''
}
...
% Appendices cut: COVID impact, sample pubs, additional teaching eval, sample
% teaching material
